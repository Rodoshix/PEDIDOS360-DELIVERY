...
import cl.duoc.pedidos360.pedidos.security.EntraTestTokens;
...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT, properties = {
 ...
 "pedidos.identidad-local.enabled=false", "spring.config.import=",
 "pedidos.interno.enabled=true",
 "pedidos.interno.tenant-id=11111111-1111-1111-1111-111111111111",
 "pedidos.interno.audience=22222222-2222-2222-2222-222222222222",
 "pedidos.interno.worker-client-id=44444444-4444-4444-4444-444444444444"
})
...
class DelegadoHttpTests {
 ...
 @Test void perfilInactivoNoAccede() throws Exception {
  active=false;
  assertThat(call("GET","/pedidos/me",EntraTestTokens.token(Map.of()),null).statusCode()).isEqualTo(403);
 }
 
 @Test void creaConPerfilRealYAislaPropietario() throws Exception {
  String token=EntraTestTokens.token(Map.of());
  var response=call("POST","/pedidos",token,"{\"restauranteId\":20,\"direccionEntrega\":\"Prueba\",\"items\":[{\"productoId\":101,\"cantidad\":2}]}");
  assertThat(response.statusCode()).isEqualTo(201);
  var json=tools.jackson.databind.json.JsonMapper.builder().build().readTree(response.body());
  assertThat(json.get("usuarioId").longValue()).isEqualTo(42);
  String path="/pedidos/"+json.get("pedidoId").longValue();
  ...
  assertThat(call("GET",path,other,null).statusCode()).isEqualTo(403);
