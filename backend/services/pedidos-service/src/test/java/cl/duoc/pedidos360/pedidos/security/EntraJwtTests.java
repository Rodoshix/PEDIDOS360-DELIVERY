...
class EntraJwtTests {
    ...
    @Test void fallaConfiguracionIncompletaOIdentidadMixta() {
        ...
        assertThatThrownBy(() -> config.entraDelegadoDecoder(new MockEnvironment())).isInstanceOf(IllegalStateException.class);
        assertThatThrownBy(() -> config.entraDelegadoDecoder(new MockEnvironment()
                .withProperty("pedidos.identidad-local.enabled", "true"))).isInstanceOf(IllegalStateException.class);
        assertThatThrownBy(() -> config.entraDelegadoDecoder(new MockEnvironment()
