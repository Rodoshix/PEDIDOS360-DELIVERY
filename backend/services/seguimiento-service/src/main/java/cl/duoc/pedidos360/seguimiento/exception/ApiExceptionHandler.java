package cl.duoc.pedidos360.seguimiento.exception;
...
public class ApiExceptionHandler {
    ...
    ProblemDetail conflicto(HttpServletRequest request) {
        return problema(HttpStatus.CONFLICT, "Los datos entran en conflicto con un seguimiento existente.", request);
    }

    @ExceptionHandler(ObjectOptimisticLockingFailureException.class)
    ProblemDetail concurrencia(HttpServletRequest request) {
        return problema(HttpStatus.CONFLICT, "El seguimiento cambió durante la operación. Vuelve a consultarlo.", request);
    }
