package cl.duoc.pedidos360.repartidores.exception;
...
public class ApiExceptionHandler {
    ...
    ProblemDetail json(HttpServletRequest request) {
        return problema(HttpStatus.BAD_REQUEST,
                "JSON inválido o campos no permitidos.", request);
    }
    ...
    ProblemDetail conflicto(HttpServletRequest request) {
        return problema(HttpStatus.CONFLICT, "Los datos entran en conflicto con un repartidor o pedido existente.", request);
    }

    @ExceptionHandler(ObjectOptimisticLockingFailureException.class)
    ProblemDetail concurrencia(HttpServletRequest request) {
        return problema(HttpStatus.CONFLICT, "El repartidor cambió durante la operación. Vuelve a consultarlo.", request);
    }
