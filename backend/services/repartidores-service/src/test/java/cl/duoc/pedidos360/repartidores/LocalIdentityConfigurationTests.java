...
import cl.duoc.pedidos360.repartidores.security.LocalIdentityConfiguration;
...
class LocalIdentityConfigurationTests {
    private final ApplicationContextRunner context = new ApplicationContextRunner()
            ...
            .withPropertyValues(
                    ...
                    "repartidores.identidad-local.roles=ADMIN",
    ...
    void permiteLocalCompletoYExigeHabilitacionExplicita() {
        ...
        context.withPropertyValues("spring.profiles.active=local", "repartidores.identidad-local.enabled=false")
                .run(result -> assertThat(result).doesNotHaveBean(IdentidadUsuario.class));
    }

    @Test
    void rechazaIdentidadIncompletaYRolesDesconocidos() {
        context.withPropertyValues("spring.profiles.active=local", "repartidores.identidad-local.object-id=")
                .run(result -> assertThat(result).hasFailed());
        context.withPropertyValues("spring.profiles.active=local", "repartidores.identidad-local.tenant-id=")
                .run(result -> assertThat(result).hasFailed());
        context.withPropertyValues("spring.profiles.active=local", "repartidores.identidad-local.roles=")
                .run(result -> assertThat(result).hasFailed());
        context.withPropertyValues("spring.profiles.active=local", "repartidores.identidad-local.roles=SUPERADMIN")
                .run(result -> assertThat(result).hasFailed());
    }
}
